...
import logging
from calcul import psnr_loss,log_mse
...
class Unet(object):
    ...
    def __init__(self,size_mask=0,channels=1, n_class=1,cost="cross_entropy", mode="abs_sp",cost_kwargs={}, **kwargs):
        ...
        print("features_root",kwargs.get("features_root",True))
        print("n_class",n_class)
        print("layers",kwargs.get("layers",True))

        with tf.variable_scope("refiner", reuse=None) as sc:
        ...
        #elif mode=="real_sp":
        #    x_sp = tf.real(fftshift_b01c(ifft3d_b01c(fftshift_b01c(self.x))))
        #    max_val=tf.reduce_max(x_sp)
        #    x_sp /=max_val
        #    #print ("max of x_sp:",np.amax(abs(x_sp)))
        #    with tf.variable_scope("refiner", reuse=None) as sc:
        #        output, _ = create_conv_net(x_sp, self.keep_prob, channels, n_class,reuse=None, **kwargs)

        #    label = tf.real(fftshift_b01c(ifft3d_b01c(fftshift_b01c(self.y))))
        #    label /=max_val
        #    self.cost = self._get_cost(output,label, cost, cost_kwargs)
    ...
    def _get_cost(self, logits,logits_label, cost_name, cost_kwargs):
        ...
        with tf.device('/cpu:0'):
            ...
            flat_labels = tf.reshape(logits_label, [-1, self.n_class])
            ...
            elif cost_name == "euclidean_norm":
                loss = 100.0*tf.reduce_mean(tf.square(tf.abs(flat_logits-flat_labels)),[0,1])/tf.reduce_mean(tf.square(tf.abs(flat_labels)),[0,1])

            elif cost_name == "euclidean":
                #loss = euclidean_loss(flat_logits,flat_labels)
                loss = 1000.0*tf.reduce_mean(tf.square(tf.abs(flat_logits-flat_labels)),[0,1])
    ...
    def predict_w_sess(self, model_path, x_test,y_dummy,sess,opt):
        """
...
class Trainer(object):
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch*training_iters), ((epoch+1)*training_iters)):
                    ...
                    #print("batch_x size", batch_x.shape)
                    #print("batch_y size", batch_y.shape)
                    #print("mask size", batch_mask.shape)

                    #print("batch_x type", batch_x.dtype)
                    #print("batch_y type", batch_y.dtype)
                    #print("mask type", batch_mask.dtype)
    ...
    def output_epoch_stats(self, epoch, total_loss, training_iters, lr):
        logging.info("Epoch {:}, Average loss: {:.5g}, learning rate: {:.3g}".format(epoch, (total_loss / training_iters), lr))
